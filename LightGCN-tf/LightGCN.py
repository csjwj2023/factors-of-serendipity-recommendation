    ...
    for epoch in range(1, args.epoch + 1):
        ...
        *********************************************************
        ...
        if ret['recall'][0] == cur_best_pre_0 and args.save_flag == 1:
            ...
            # 获取用户和物品嵌入矩阵的变量
            user_embedding_matrix = sess.run(model.weights['user_embedding'])
            item_embedding_matrix = sess.run(model.weights['item_embedding'])
            # 保存用户和物品嵌入矩阵到npy文件
            print("===========user_embedding_matrix:\n",user_embedding_matrix)
            print("===========user_embedding_matrix.shape:\n", user_embedding_matrix.shape)
            np.save(weights_save_path + '/emb_user.npy', user_embedding_matrix)
            np.save(weights_save_path + '/emb_item.npy', item_embedding_matrix)
