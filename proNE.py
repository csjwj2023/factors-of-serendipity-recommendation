...
def parse_args(input,out_emb1,out_emb2):
    parser = argparse.ArgumentParser(description="Run ProNE.")
    parser.add_argument('-graph', nargs='?', default=input,
                        help='Graph path')
    parser.add_argument('-emb1', nargs='?', default=out_emb1,
                        help='Output path of sparse embeddings')
    parser.add_argument('-emb2', nargs='?', default=out_emb2,
...
def main():
    dataset_name='beauty'
    input_graph=os.path.join("data", dataset_name, "proNE_ui_graph.txt")
    out_emb1=os.path.join("data", dataset_name, "proNE_ui_emb.npy")
    out_emb2=os.path.join("data", dataset_name, "proNE_ui_emb_en.npy")

    args = parse_args(input_graph,out_emb1,out_emb2)
    t_0 = time.time()
    ...
    np.save(out_emb1, features_matrix)
    np.save(out_emb2, embeddings_matrix)

    import pandas as pd
    train_data = pd.read_csv(input_graph, header=None,sep=' ')
    print(train_data)
    train_data.columns = ['userInd', 'itemGid']
    itemInd2GidOffset=max(train_data["userInd"])+1

    userEmd=embeddings_matrix[:itemInd2GidOffset,:]
    itemEmd=embeddings_matrix[itemInd2GidOffset:,:]
    np.save(os.path.join("data", dataset_name, "emb_user_proNE.npy"), userEmd)
    np.save(os.path.join("data", dataset_name, "emb_item_proNE.npy"), itemEmd)
    print(userEmd)
    print(userEmd.shape)
    print(itemEmd.shape)
